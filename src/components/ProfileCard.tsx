    ...
    >
      <Link href={`/era/${era.id}`} className="group block text-center">
        <motion.div
          ...
          className="relative mx-auto aspect-square w-32 overflow-hidden rounded-sm border-4 border-transparent bg-zinc-800 transition duration-200 group-hover:border-white sm:w-36 md:w-44"
        >
        ...
        <p className="mt-3 text-base font-semibold text-[#9b9b9b] transition group-hover:text-white sm:text-xl">{era.title}</p>
        <p className="mx-auto mt-1 max-w-[11rem] text-xs font-medium uppercase tracking-wide text-zinc-600 transition group-hover:text-zinc-400">
          {era.subtitle}
        </p>
      </Link>
    </motion.div>
